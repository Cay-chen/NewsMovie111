    ...
    private void setAllData(final YouShiMovieDealisBean subjectsBean) {
        ...
        if (subjectsBean.getSubtype().equals("1")) {
            if (subjectsBean.getNow_num().equals(subjectsBean.getTotal_num())) {
                bindingContentView.btnYoushiMovieDetail.setText(subjectsBean.getNow_num() + "集全");
            } else {
                bindingContentView.btnYoushiMovieDetail.setText("已更新至" + subjectsBean.getNow_num() + "集");
            }
        } else {
            bindingContentView.btnYoushiMovieDetail.setText("高清中字");
        }
